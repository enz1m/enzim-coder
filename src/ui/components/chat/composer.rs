...
fn worktree_merge_action_label(action: &crate::worktree::WorktreeMergeAction) -> &'static str {
    match action {
        crate::worktree::WorktreeMergeAction::Write => "Update",
        crate::worktree::WorktreeMergeAction::Delete => "Delete",
        crate::worktree::WorktreeMergeAction::Rename => "Rename",
    }
}
...
pub(super) fn open_worktree_merge_popup(
    ...
    let preview = match crate::worktree::preview_worktree_merge(worktree_path) {
        Ok(preview) => preview,
        Err(err) => {
...

    {
        ...
        merge.connect_clicked(move |_| {
            ...
            let (tx, rx) = mpsc::channel::<Result<crate::worktree::WorktreeMergeResult, String>>();
            let worktree_path_bg = worktree_path_for_apply.clone();
            let live_workspace_path_bg = live_workspace_path_for_apply.clone();
            thread::spawn(move || {
                let result = crate::worktree::apply_worktree_merge(
                    &worktree_path_bg,
                    &live_workspace_path_bg,
                )
                .and_then(|merge_result| {
                    crate::worktree::stop_worktree_checkout(&worktree_path_bg)?;
                    Ok(merge_result)
                });
