...
fn start_account_sync_loop(db: Rc<AppDb>, manager: Rc<CodexProfileManager>) {
    let (tx, rx) = mpsc::channel::<(i64, Option<crate::codex_appserver::AccountInfo>)>();
...
fn start_remote_thread_activation_loop(
    ...
    crate::ui::scheduler::every(Duration::from_millis(120), move || {
        let Some(raw_thread_id) = db
            .get_setting(crate::remote::SETTING_REMOTE_TELEGRAM_ACTIVATE_LOCAL_THREAD_ID)
            .ok()
            .flatten()
...
pub fn build_ui(app: &adw::Application) {
    ...
    let app_data_dir = crate::data::default_app_data_dir();
