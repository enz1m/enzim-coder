    ...
    match method {
                ...
                "turn/completed" => {
                    if let Some(turn_id) = super::codex_events::extract_turn_id(&event.params) {
                        ...
                        if let Some(thread_id) = resolved_thread_id.as_deref() {
                            crate::services::app::remote::forward_turn_completion_if_enabled(
                                db.as_ref(),
                                thread_id,
                                &turn_id,
                        ...
                        if is_active_thread {
                            if let Some(thread_id) = resolved_thread_id.as_deref() {
                                ...
                                let checkpoint_id = crate::services::app::restore::capture_turn_checkpoint(
                                    &db,
                                    thread_id,
                                    &turn_id,
                                    &all_file_changes,
                                ...
                                if checkpoint_id.is_none() && has_write_like_command {
                                    ...
                                    thread::spawn(move || {
                                        let checkpoint_id = crate::services::app::chat::BackgroundRepo::capture_workspace_delta_checkpoint_for_remote_thread(
