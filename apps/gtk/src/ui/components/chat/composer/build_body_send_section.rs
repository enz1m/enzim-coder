...

    {
        ...
        send.connect_clicked(move |_| {
            let skills_allowed_for_text = |text: &str| -> Result<(), String> {
                ...
                let assignments =
                    crate::services::app::skills::load_profile_assignments(db.as_ref(), profile_id);
                let blocked = crate::services::app::skills::disabled_skill_markers(
                    text,
                    &catalog,
                    &assignments,
                );
            ...
            if is_turn_in_progress {
                if !text.trim().is_empty() || has_images {
                    ...
                    steer_button.connect_clicked(move |_| {
                        ...
                        thread::spawn(move || {
                            let _ = crate::services::app::chat::BackgroundRepo::ensure_remote_thread_baseline_checkpoint(&baseline_thread_id);
                        });
                            ...
                            Rc::new(move |client: Arc<RuntimeClient>| {
                                ...
                                std::thread::spawn(move || {
                                    ...
                                    let workspace_path_for_turn =
                                        crate::services::app::chat::BackgroundRepo::workspace_path_for_remote_thread(&thread_id);
