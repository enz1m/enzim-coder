...
fn active_branch_label(db: &AppDb) -> Option<String> {
    ...
    let workspace_path = db
        ...
        .ok()
        .flatten()?;
    let workspace_root = crate::services::ops::git::run_git_text(
    ...
    crate::services::ops::git::run_git_text(&repo_root, &["symbolic-ref", "--short", "HEAD"])
        .ok()
        .map(|value| value.trim().to_string())
        .filter(|value| !value.is_empty())
        .or_else(|| {
            crate::services::ops::git::run_git_text(&repo_root, &["rev-parse", "--short", "HEAD"])
                .ok()
