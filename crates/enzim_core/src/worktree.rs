...
fn prune_empty_worktree_parents(path: &Path, app_data_dir: &Path) {
    let stop = app_data_dir.join("worktrees");
...
pub fn stop_worktree_checkout(worktree_path: &str, app_data_dir: &Path) -> Result<(), String> {
    let worktree_root = PathBuf::from(worktree_path);
    if !worktree_root.exists() {
        return Ok(());
    }
    ...
    match fs::remove_dir_all(&worktree_root) {
        Ok(_) => {
            prune_empty_worktree_parents(&worktree_root, app_data_dir);
            Ok(())
        }
        Err(err) if err.kind() == io::ErrorKind::NotFound => {
            prune_empty_worktree_parents(&worktree_root, app_data_dir);
